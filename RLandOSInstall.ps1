#RLand OS Install V8
Read-Host -Prompt 'Bienvenido al Instalador de RLand OS By TheRodrike
...
$teamspeak = 'Desea Descargar TeamSpeak (Comunicacion)?'
...
$tb = 'Desea Revisar el github de TranslucentTB (Taskbar)?'
...
Write-Host 'Configuracion del Sistema Finalizada'
Read-Host -Prompt 'Pulse Enter'
...
if ($decision -eq 0) {
    ...
    if ($decision -eq 0) {
        ...
        $decision = $Host.UI.PromptForChoice($title, $teamspeak, $choices, 0)
        if ($decision -eq 0) {
            Start-Process 'https://teamspeak.com/es/downloads/'
        } else {
            Write-Host 'No'
        }
    } else {
    ...
    if ($decision -eq 0) {
        ...
        $decision = $Host.UI.PromptForChoice($title, $tb, $choices, 0)
        if ($decision -eq 0) {
...
#Finalizar
Write-Host 'Gracias por Instalar RLand OS'
Read-Host -Prompt 'Pulse Enter para Finalizar'
